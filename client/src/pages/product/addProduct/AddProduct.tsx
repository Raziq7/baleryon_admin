...
const AddProduct = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prevData) => {
      // If the user is updating the discount amount
      if (name === "discount") {
        const price = parseFloat(prevData.price);
        const discountAmount = parseFloat(value);

        // Only calculate if price is a valid number
        ...
        return {
          ...prevData,
          discount: value, // Show amount in input
          discountPercentage: discountPercentage,
        };
      }

      // If user is updating price, and discount is already entered, recalculate percentage
      if (name === "price") {
        const price = parseFloat(value);
        const discountAmount = parseFloat(prevData.discount);
  ...
  const colorPickHandler = (e) => {
    ...
    if (isValidHex(value)) {
      ...
      matched = Object.keys(colors).find((color) =>
        color.startsWith(value.toLowerCase())
      );
            ...
            <Grid item xs={12} md={6}>
              ...
              {formData.discountPercentage && (
                <Typography variant="caption" color="textSecondary">
                  Discount: {formData.discountPercentage}% of price
                </Typography>
              )}
            </Grid>
